...
{
  /* [ptr] should not be null, should be in user page_address and in page */
  ...
  {
    if (!try_fetch_page((void *)ptr, checker_esp))
      syscall_exit(-1);
  }
  #endif
}
...
{
  /* First check the start page_address */
  check_valid_ptr(str);
  /* Then check the whole page_address of the string */
  while (*str)
    check_valid_ptr(++str);
}
...
int syscall_exec(const char *cmd_line)
{
  ...
  sem_down(&child_process->load_sema);
  /* Ensure load success */
  /* Note that process can be exited */
  bool success = child_process->status == PROCESS_RUNNING || child_process->status == PROCESS_EXITED;
  if (!success)
  {
    /* Load failed */
...
{
  /* Target memory page_address should be valid */
...
{
  /* Source mem page_address should be valid */
  check_valid_mem(buffer, size);
  if (fd == STDOUT_FILENO)
  {
...
static mmap_entry_t *
new_mmap_entry(void *page_address, struct file *file, int page_count)
{
...
#ifdef VM
  struct thread *cur = thread_current();
  void *page_address = entry->page_address;
  /* Iterate through all mapped page and release resources */
  for (int cur_page = 0; cur_page < entry->page_count; ++cur_page)
  {
    sup_page_table_entry_t *table_entry = sup_table_find(&cur->sup_page_table, page_address);
    if (table_entry)
    {
      /* If dirty: write back */
      if (pagedir_is_dirty(cur->pagedir, page_address))
      {
        lock_acquire(&filesys_lock);
        file_seek(table_entry->file, table_entry->ofs);
        file_write(table_entry->file, page_address, table_entry->read_bytes);
        lock_release(&filesys_lock);
      }
      /* Delete from page table */
      if (pagedir_get_page(cur->pagedir, table_entry->page_address))
      {
        release_frame(
            pagedir_get_page(cur->pagedir, table_entry->page_address));
        pagedir_clear_page(cur->pagedir, table_entry->page_address);
      }
...
static bool
check_mmap_overlaps(void *page_address, int size)
{
#ifdef VM
  if (!page_address || size < 0)
    return false;
  struct thread *cur = thread_current();
  ...
  {
    if (sup_table_find(&cur->sup_page_table, page_address) || pagedir_get_page(cur->pagedir, page_address))
      return false;
    page_address += PGSIZE;
  }
  return true;
...
mapid_t
syscall_mmap(int fd, void *page_address)
{
#ifdef VM
  /* Console or not multiples of page size */
  if (fd < 2 || (uint32_t)page_address % PGSIZE)
    return -1;
  ...
  if (!f)
    return -1;
  /* Check if page_address has overlaps */
  if (!check_mmap_overlaps(page_address, file_size))
  {
    return -1;
  }
  ...
  mmap_entry_t *mmap_entry = new_mmap_entry(page_address, f, page_count);
  /* Lazy load the file from disk */
  if (!lazy_load(f, 0, page_address, read_bytes, zero_bytes, true, true))
  {
    free(mmap_entry);
    return -1;
  }
  /* Add mmap entry into mmap list */
  list_ins_back(&thread_current()->mmap_list, &mmap_entry->elem);
